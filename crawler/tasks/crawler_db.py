...
class DBStoreTask(luigi.Task):
    ...
    def run(self):
        ...
        if not post_url:
            print("No DIRECTORY_STORE_URL configured in instance/config.cfg, exiting.")
            exit(1)
    ...
    def output(self):
        return luigi.LocalTarget("data/%s_db.txt" % self.accession_number)
